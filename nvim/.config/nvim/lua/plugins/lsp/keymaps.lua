...
function M.get()
	...
	if require("util.lazyvim").has("inc-rename.nvim") then
		M._keys[#M._keys + 1] = {
			"<leader>cr",
			function()
...
function M.has(buffer, method)
	...
	local clients = require("util.lazyvim").lsp.get_clients({ bufnr = buffer })
...
function M.resolve(buffer)
	...
	local spec = M.get()
	local opts = require("util.lazyvim").opts("nvim-lspconfig")
	local clients = require("util.lazyvim").lsp.get_clients({ bufnr = buffer })
	for _, client in ipairs(clients) do
